...
This guide provides a detailed engineering analysis of the self-hosted infrastructure stack required for the **RAG Eval Lab**. It covers service dependencies, database engines, caching layer benefits, and step-by-step instructions to spin up the entire cluster using Docker Compose.
...
graph TD
    subgraph Local Client
        CLI[Python main.py] -->|Retrieval / Embed| Ollama(Ollama Embeddings API)
    end

    subgraph Docker Virtual Network
        CLI -->|Hybrid Search| Qdrant[(Qdrant Vector DB: 6333)]
        CLI -->|Telemetry Traces| LF[Langfuse Web Server: 3000]
...
* **Langfuse Observer Dashboard**: Open `http://localhost:3000` in your web browser. 
  ...
  * Go to **Settings** $\rightarrow$ **API Credentials** $\rightarrow$ **Create API Keys**.
  * Copy the **Public Key**, **Secret Key**, and **Host** (`http://localhost:3000`) and paste them directly into your project's `.env` file!
